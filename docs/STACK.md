...
| 폰트          | LINE Seed KR(로컬, `src/fonts/`)/JP(`@fontsource/line-seed-jp`), IBM Plex Mono·RocknRoll One(Google Fonts) — 851MkPOP(로컬 폰트) |
| 데이터베이스  | Supabase (`src/lib/supabase/`) — zutopia 곡/앨범 DB. `output: "export"` 정적 빌드라 빌드 타임 전용 클라이언트만 사용             |
