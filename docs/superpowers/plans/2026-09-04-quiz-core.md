...
- Produces: `supabase/config.toml`(이후 태스크의 마이그레이션이 참조), `.env.local`의 `NEXT_PUBLIC_SUPABASE_URL`/`NEXT_PUBLIC_SUPABASE_ANON_KEY`(Task 7에서 소비)
...
**이 단계는 실행자가 본인의 Supabase 프로젝트 값으로 직접 채워야 한다** — Supabase 대시보드의 Project Settings → API에서 URL과 `anon` `public` key를 복사.
...
- Consumes: Task 1의 `.env.local` (`NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`)
