import type { Song, SongTitle } from "@/features/guide/lib/types";

const kira: SongTitle = {
  jp: "綺羅キラー (feat. Mori Calliope)",
  kr: "키라 킬러",
  en: "Kira Killer",
};

const song: Song = {
  id: "kira-killer",
  title: kira,
  youtubeId: "HZK_9uWAqeo",
  lyrics: [
    {
      time: "0:0",
      original: "♫",
      pronunciation: "",
      translation: "",
      interlude: true,
    },
    //================ 1절 ================//
    {
      time: "0:13.5",
      original: "周りに合わせる言語も知らねえ",
      pronunciation: "마와리니 아와세루 겐고모 시라네",
      translation: "주위에 맞춰줄 언어도 몰라",
    },
    {
      time: "0:15.2",
      original: "守備力は弱いけど勘は鋭",
      pronunciation: "슈비료쿠와 요와이케도 칸와 에-",
      translation: "수비력은 약하지만 감은 좋아",
    },
    {
      time: "0:16.7",
      original: "一旦進められたら止まれ",
      pronunciation: "잇탄 스스메라레타라 토마레",
      translation: "일단 넘어갔으면 멈춰",
    },
    {
      time: "0:18.5",
      original: "いったん木綿みたいに",
      pronunciation: "잇탄 모멘 미타이니",
      translation: "잇탄모멘처럼",
    },
    {
      time: "0:20",
      original: "曲がれまだ上 ゆらゆら揺らして",
      pronunciation: "마가레 마다 우에 유라유라 유라시테",
      translation: "꺾어, 더 위로 흔들흔들 흔들면서",
    },
    {
      time: "0:21.8",
      original: "すぐとらぶるぜ 過呼吸と心得",
      pronunciation: "스구 토라부루제 브레스토 코코로엣",
      translation: "금세 트러블이 생겨 과호흡과 수칙",
    },
    {
      time: "0:23.5",
      original: "って原罪人生 盾と矛で 葛藤の活動がマスト",
      pronunciation:
        "테 겐자이 진세이 타테토 호코데 캇토우노 카츠도우가 마스토",
      translation: "원죄 인생, 방패와 창으로 갈등 활동이 필수",
    },
    {
      time: "0:27.1",
      original: "締め切られなきゃ言葉が増殖",
      pronunciation: "시메키라레나캬 코토바가 조우쇼쿠",
      translation: "닫아 두지 않으면 말이 증식할 거야",
    },
    {
      time: "0:30.2",
      original: "No thoughts, head empty 虚無で円陣",
      pronunciation: "No thoughts, head empty 쿄무데 엔진",
      translation: "No thoughts, head empty 둘러싼 허무",
    },
    {
      time: "0:32.2",
      original: "Brought to you in chaos and serenity",
      pronunciation: "Brought to you in chaos and serenity",
      translation: "혼돈과 평온 속에서 네게로 가져온 것",
    },
    {
      time: "0:33.4",
      original: "やめたいことをやらなきゃ満たされない僕",
      pronunciation: "야메타이 코토오 야라나캬 미타사레나이 보쿠",
      translation: "그만두고 싶은 것을 해야만 만족하는 나",
    },
    {
      time: "0:37",
      original: "Shockin’, isn’t it?",
      pronunciation: "Shockin’, isn’t it?",
      translation: "놀랍지, 그치?",
    },
    {
      time: "0:38",
      original: "“Dream life?” You’re livin’ it",
      pronunciation: "“Dream life?” You’re livin’ it",
      translation: '"드림라이프?" 즐기고 있잖아',
    },
    {
      time: "0:38.8",
      original: "“バイトだったらどこでもよくない？”",
      pronunciation: "바이토닷타라 도코데모 요쿠나이",
      translation: '"알바야 어디라도 상관 없지 않아?"',
    },
    {
      time: "0:40.5",
      original: "綺羅キラー",
      pronunciation: "키라 키라-",
      translation: "키라 킬러",
      cheer: {
        text: "V자로 흔들기 반복",
        tag: "swing",
      },
    },
    {
      time: "0:42",
      original: "嫌いなんて序の口 そう思うと楽だし",
      pronunciation: "키라이난테 죠노 쿠치 소우 오모우토 라쿠다시",
      translation: "싫은 건 시작에 불과해 그렇게 생각하면 편해",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "0:45.3",
      original: "3.2.1で歪み合った",
      pronunciation: "3.2.1데 이가미앗타",
      translation: "3.2.1로 뒤틀려",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "0:48.8",
      original: "君のその口 シガレット辛口",
      pronunciation: "키미노 소노 쿠치 시가렛토 카라쿠치",
      translation: "너의 그 입에 톡 쏘는 시가렛",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "0:51.9",
      original: "割り切ったけど",
      pronunciation: "와리킷타케도",
      translation: "분명 체념했는데",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "0:53.9",
      original: "知るか? 汁なら drink up we’re still tough",
      pronunciation: "시루카 시루나라 drink up we’re still tough",
      translation: "알 바야? 국이라면 drink up we’re still tough",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "0:55.5",
      original: "最低なコンプだし 最高の昆布だし",
      pronunciation: "사이테이나 콘푸다시 사이코우노 콘부다시",
      translation: "최악의 콤프고, 최고의 육수고",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "0:58.8",
      original: "揃ってるだけじゃつまらんし",
      pronunciation: "소롯테루 다케쟈 츠마란시",
      translation: "모여 있는 것만으론 재미가 없고",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "1:02.3",
      original: "繋いだって綻ぶし しょっぱいな鰹節",
      pronunciation: "츠나이닷테 호코로부시 숏파이나 가츠오부시",
      translation: "이어 봤자 풀리고 짭짤한 가다랑어포",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "1:05.45",
      original: "もっと甘えたいけどー？",
      pronunciation: "못토 아마에타이케도-?",
      translation: "좀 더 응석부리고 싶은데-？",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "1:08.5",
      original: "ぞんざいでアップダウンな現状が今ヒット中",
      pronunciation: "존자이데 앗푸다운나 겐죠우가 이마 힛토 츄",
      translation: "소홀하고 업다운도 있는 현상이 지금 히트 중",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "1:12.2",
      original: "落ち込んでる方が 進めるセオリー",
      pronunciation: "오치콘데루 호우가 스스메루 세오리-",
      translation: "침울해하면 더 진행되는 시어리",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "1:15.6",
      original: "うpダウンな成長が いまひとつ",
      pronunciation: "앗푸다운나 세이쵸우가 이마 히토츠",
      translation: "업다운의 성장이 지금 하나",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "1:19.1",
      original: "(底辺のてっぺんの味で 満たされたいわ)",
      pronunciation: "(테이헨노 텟펜노 아지데 미타사레타이와)",
      translation: "(밑바닥 정상의 맛으로 충족되고 싶어)",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "1:25.6",
      original: "♫",
      pronunciation: "",
      translation: "",
      interlude: true,
    },
    //================ RAP ================//
    {
      time: "1:36",
      original: "",
      pronunciation: "I don’t wanna write, or hold a mic Feedback stopped me",
      translation:
        "글을 쓰거나 마이크를 잡고 싶은 게 아냐 피드백은 날 멈추게 했으니까",
    },
    {
      time: "1:38",
      original: "",
      pronunciation: "I fell off, I gotta buy brand coffee",
      translation: "떨어졌으니 커피나 사야겠어",
    },
    {
      time: "1:39.5",
      original: "",
      pronunciation: "Figured the days of genre were long gone",
      translation: "장르의 시대는 이미 지났다고 생각했는데",
    },
    {
      time: "1:41.3",
      original: "Thanks a lot 君のsongが好きそう",
      pronunciation: "Thanks a lot 키미노 song가 스키소우",
      translation: "Thanks a lot 네 song이 좋은 것 같아",
    },
    {
      time: "1:43",
      original: "Though I’m washed up 未だ足りない",
      pronunciation: "Though I’m washed up 이마다 타리나이",
      translation: "Though I’m washed up 아직 부족해",
    },
    {
      time: "1:44.6",
      original: "",
      pronunciation: "Throwin’ lyrics like expired canned food stuff",
      translation: "유통기한 지난 통조림 같은 가사",
    },
    {
      time: "1:46.4",
      original: "",
      pronunciation: "Average kid got too big, I’m toast",
      translation: "애들 평균이 너무 커졌어, 난 끝장이야",
    },
    {
      time: "1:47.9",
      original: "",
      pronunciation:
        "Litter the internet with bitter sentiment and move the goal post",
      translation: "씁쓸한 마음으로 인터넷을 어지럽히고 규칙을 몰래 바꿔",
    },
    {
      time: "1:49.7",
      original: "ぐずぐず おセンチな マイナス思考だ",
      pronunciation: "구즈구즈 오센치나 마이나스 시코다",
      translation: "투덜투덜 센치한 마이너스 사고야",
    },
    {
      time: "1:51.5",
      original: "",
      pronunciation: "Nobody’s coming and still I’m crying “Red Rover” (What?)",
      translation: '아무도 오지 않지만 여전히 "레드 로버"를 외쳐 (뭐?)',
    },
    {
      time: "1:53",
      original: "",
      pronunciation: "Not every line hits",
      translation: "모든 대사가 히트를 치진 않아",
    },
    {
      time: "1:54",
      original: "",
      pronunciation: "Moments of “I miss…”",
      translation: '"그리워…" 의 순간들',
    },
    {
      time: "1:54.9",
      original: "",
      pronunciation: "Being nobody cuz nobody owns kindness",
      translation: "누구도 아닌 이유는 누구도 다정을 소유할 수 없기 때문이지",
    },
    {
      time: "1:56.8",
      original: "Should let myself slack OFF みんなの前で",
      pronunciation: "Should let myself slack OFF 민나노 마에데",
      translation: "내가 알아서 할 거야 slack OFF 모두 앞에서",
    },
    {
      time: "1:57.9",
      original: "",
      pronunciation: "Might go back to being unknown. Know what? Fine",
      translation: "다시 무명 신세가 될지도 몰라, 알지? 괜찮아",
    },
    {
      time: "1:59.7",
      original: "",
      pronunciation: "In the grand scheme, man dreams, never needing sleep aid",
      translation:
        "원대한 계획에서 꿈꾸는 사람에겐, 절대 수면제가 필요하지 않아",
    },
    {
      time: "2:01.5",
      original: "“しょっぱい夢” さよなら 会えてよかった",
      pronunciation: "숏파이 유메 사요나라 아에테 요캇타",
      translation: '"짭짤한 꿈"은 작별이야 만나서 다행이야',
    },
    //================ 2절 ================//
    {
      time: "2:03.3",
      original: "キラキラ",
      pronunciation: "키라키라",
      translation: "반짝반짝",
      cheer: {
        text: "V자로 흔들기 반복",
        tag: "swing",
      },
    },
    {
      time: "2:04.9",
      original: "嫌いなんて序の口 そう思うと楽だし",
      pronunciation: "키라이난테 죠노 쿠치 소우 오모우토 라쿠다시",
      translation: "싫은 건 시작에 불과해 그렇게 생각하면 편해",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "2:08.2",
      original: "3.2.1で歪み合った",
      pronunciation: "3.2.1데 이가미앗타",
      translation: "3.2.1로 뒤틀려",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "2:11.5",
      original: "君のその口 シガレット辛口",
      pronunciation: "키미노 소노 쿠치 시가렛토 카라쿠치",
      translation: "너의 그 입에 톡 쏘는 시가렛",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "2:14.9",
      original: "割り切ったけど",
      pronunciation: "와리킷타케도",
      translation: "분명 체념했는데",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "2:16.8",
      original: "知るか? 汁なら drink up we’re still tough",
      pronunciation: "시루카 시루나라 drink up we’re still tough",
      translation: "알 바야? 국이라면 drink up we’re still tough",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "2:18.5",
      original: "最低なコンプだし 最高の昆布だし",
      pronunciation: "사이테이나 콘푸다시 사이코우노 콘부다시",
      translation: "최악의 콤프고, 최고의 육수고",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "2:21.7",
      original: "揃ってるだけじゃつまらんし",
      pronunciation: "소롯테루 다케쟈 츠마란시",
      translation: "모여 있는 것만으론 재미가 없고",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "2:25.25",
      original: "繋いだって綻ぶし しょっぱいな鰹節",
      pronunciation: "츠나이닷테 호코로부시 숏파이나 가츠오부시",
      translation: "이어 봤자 풀리고 짭짤한 가다랑어포",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "2:28.5",
      original: "もっと甘えたいけどー？",
      pronunciation: "못토 아마에타이케도-?",
      translation: "좀 더 응석부리고 싶은데-？",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "2:33.7",
      original: "",
      pronunciation: "",
      translation: "주렁주렁",
      interlude: true,
    },
    //================ 3절 ================//
    {
      time: "2:47.7",
      original: "出だしの荒削り衝動 ここにある",
      pronunciation: "데다시노 아라케즈리 쇼우도우 코코니 아루",
      translation: "맨 처음의 조잡함과 충동이 여기에 있어",
    },
    {
      time: "2:53.3",
      original: "学んでるようで 学んでない声してる",
      pronunciation: "마난데루요우데 마난데나이 코에 시테루",
      translation: "배운 것 같으면서도 배우지 않은 목소리를 하고 있어",
    },
    {
      time: "2:56.6",
      original: "期限切れ らいか ろんりねす 受け入れるよ",
      pronunciation: "키겐키레 라이카 론리네스 우케이레루요",
      translation: "지난 기한, 내년 여름, 외로움, 받아들일게",
    },
    {
      time: "3:01",
      original: "素顔を君に送りたいけど",
      pronunciation: "스가오오 키미니 오쿠리타이케도",
      translation: "민낯을 네게 보내주고 싶은데",
    },
    {
      time: "3:06",
      original: "飽きられてしまうから 糧にする",
      pronunciation: "아키라레테 시마우카라 카테니 스루",
      translation: "싫증날 테니까 밑거름 삼을게",
    },
    {
      time: "3:10.1",
      original: "闇エネルギーは いつも助かってるんだよ",
      pronunciation: "야미- 에네르기-와 이츠모 타스캇테룬다요",
      translation: "어둠 에너지는 언제나 도움을 받고 있으니까",
    },
    {
      time: "3:17.5",
      original: "綺羅綺羅",
      pronunciation: "키라키라",
      translation: "기라기라",
      cheer: {
        text: "V자로 흔들기 반복",
        tag: "swing",
      },
    },
    {
      time: "3:19.2",
      original: "羅鬼羅鬼",
      pronunciation: "라키라키",
      translation: "Like it Like it",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "3:21",
      original: "きらーーいなんて序の口 そう思うと楽だし",
      pronunciation: "키라--이난테 죠노 쿠치 소우 오모우토 라쿠다시",
      translation: "싫어한다는 건 시작에 불과해 그렇게 생각하면 편해",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "3:24.3",
      original: "3.2.1で歪み合った",
      pronunciation: "3.2.1데 이가미앗타",
      translation: "3.2.1로 뒤틀려",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "3:27.5",
      original: "君のその口 シガレット辛口",
      pronunciation: "키미노 소노 쿠치 시가렛토 카라쿠치",
      translation: "너의 그 입에 톡 쏘는 시가렛",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "3:30.8",
      original: "割り切ったけど",
      pronunciation: "와리킷타케도",
      translation: "분명 체념했는데",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "3:32.8",
      original: "知るか? 汁なら drink up we’re still tough",
      pronunciation: "시루카 시루나라 drink up we’re still tough",
      translation: "알 바야? 국이라면 drink up we’re still tough",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "3:34.5",
      original: "いつだって研修生 価値が小惑星",
      pronunciation: "이츠닷테 켄슈우세이 카치가 쇼우와쿠세이",
      translation: "언제나 연수생, 가치가 소행성",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "3:37.9",
      original: "乗りこなす必要もないし",
      pronunciation: "노리코나스 히츠요우모 나이시",
      translation: "잘 탈 필요도 없으니까",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "3:41.2",
      original: "君と共感勝ち 噛めば音の だち",
      pronunciation: "키미토 쿄우칸 카치 카메바 오토노 다치",
      translation: "너와의 공감이 승리, 깨물면 나는 소리",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "3:44.7",
      original: "もっと甘えたいけどーー？",
      pronunciation: "못토 아마에타이케도--?",
      translation: "좀 더 응석부리고 싶은데--？",
      cheer: {
        text: "",
        tag: "swing",
      },
    },
    {
      time: "3:49",
      original: "♫",
      pronunciation: "",
      translation: "",
      interlude: true,
    },
    {
      time: "4:01.6",
      original: "",
      pronunciation: "",
      translation: "주렁주렁",
      interlude: true,
    },
    {
      time: "4:08.5",
      original: "♫",
      pronunciation: "",
      translation: "",
      interlude: true,
    },
  ],
};

export default song;
